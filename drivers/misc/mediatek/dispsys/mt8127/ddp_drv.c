...
//    unsigned int status;
    ...
    switch(cmd)
    {
        case DISP_IOCTL_WRITE_REG:
			...
			{
                DISP_ERR("reg write, addr invalid, addr=0x%x \n", wParams.reg);
                return -EFAULT;
			}
...
//            cmq_status[value] = 1;
        ...
        */
        ...
        case DISP_IOCTL_GET_OVL:
            ...
            {
                DISP_ERR("DISP_IOCTL_GET_OVL, copy_from_user failed, %d\n", ret);
                return -EFAULT;
            }
            layer = ovl_info.layer;
			if(layer >= DDP_OVL_LAYER_MUN || layer < 0){   //idx should be between 0-3
				DISP_ERR("DISP_IOCTL_GET_OVL, invalid layer %d\n", layer);
                return -EFAULT;
			}
			spin_lock(&gOvlLock);
