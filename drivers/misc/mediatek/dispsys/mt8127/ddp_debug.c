...
static void process_dbg_opt(const char *opt)
{
    ...
    {
        char *p = (char *)opt + 12;
        unsigned int value = (unsigned int)simple_strtoul(p, &p, 10);
        ...
        ret = KREE_TeeServiceCall(ddp_session_handle(), TZCMD_DDP_SET_DEBUG_LOG, paramTypes, param);
...
{  
    ...
    disp_power_on(DISP_MODULE_WDMA); 
   */
    ...
    disp_power_off(DISP_MODULE_WDMA);
    */
    //dealloc memory
    vfree(pSrc);
    vfree(pDst);   
...
char* ddp_get_module_name(DISP_MODULE_ENUM module)
{
    switch(module)
    {
        ...
        case DISP_MODULE_WDMA:   return "wdma0";
...
char* ddp_get_fmt_name(DISP_MODULE_ENUM module, unsigned int fmt)
{
   ...
   else if(module==DISP_MODULE_RDMA0 || module==DISP_MODULE_RDMA1) // todo: confirm with designers
   {
       switch(fmt)
       {
           case 0:  return "yuyv";
    ...
    {        
        ...
        case DISP_MODULE_WDMA:  
            DISP_MSG("wdma0: en=%d, w=%d, h=%d, clip=(%d, %d, %d, %d), pitch=(%d,%d), addr=(0x%x,0x%x,0x%x), fmt=%s \n",
              ...
              ddp_get_fmt_name(DISP_MODULE_WDMA, (DISP_REG_GET(DISP_REG_WDMA_CFG)>>4)&0xf));
        ...
        case DISP_MODULE_RDMA0:  
             ...
             DISP_MSG("rdma%d: en=%d, mode=%s, smi_busy=%d, w=%d, h=%d, pitch=%d, addr=0x%x, fmt=%s, fifo_min=%d \n",
              ...
              ((DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON+DISP_INDEX_OFFSET*index)&0x2)==0) ? "rgb888" : ddp_get_fmt_name(DISP_MODULE_RDMA0, (DISP_REG_GET(DISP_REG_RDMA_MEM_CON+DISP_INDEX_OFFSET*index)>>4)&0x3f),
        ...
        case DISP_MODULE_RDMA1:  
             ...
             DISP_MSG("rdma%d: en=%d, mode=%s, smi_busy=%d, w=%d, h=%d, pitch=%d, addr=0x%x, fmt=%s, fifo_min=%d \n",
              ...
              DISP_REG_GET(DISP_REG_RDMA_MEM_SRC_PITCH+DISP_INDEX_OFFSET*index), 
              DISP_REG_GET(DISP_REG_RDMA_MEM_START_ADDR+DISP_INDEX_OFFSET*index),
              ((DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON+DISP_INDEX_OFFSET*index)&0x2)==0) ? "rgb888" : ddp_get_fmt_name(DISP_MODULE_RDMA1, (DISP_REG_GET(DISP_REG_RDMA_MEM_CON+DISP_INDEX_OFFSET*index)>>4)&0x3f),
              DISP_REG_GET(DISP_REG_RDMA_FIFO_LOG+DISP_INDEX_OFFSET*index));
            DISP_MSG("rdma%d: ack=%d, req=%d, status=%s, output_x=%d, output_y=%d \n", 
              index, 
              (DISP_REG_GET(DISPSYS_RDMA1_BASE+0x400)>>10)&0x1,
