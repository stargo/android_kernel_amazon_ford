...
static struct i2c_board_info __initdata fts_i2c_tpd = {
	I2C_BOARD_INFO("fts", (0x70 >> 1)) };
		 ...
		 char *readbuf, int readlen)
{
	...
	if (readlen != 0) {
		...
		ret =
		    i2c_master_recv(client, (unsigned char *)g_dma_buff_pa,
				    readlen);
...
static int fts_report_value(struct ts_event *data)
{
	...
	for (i = 0; i < fts_updateinfo_curr.TPD_MAX_POINTS; i++) {
		...
		if ((data->au8_touch_event[i] == 0
			...
			|| (data->pressure[i] == 0 && data->area[i] == 0)))
			return 1;
		/*
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (data->au8_touch_event[i] == 0
			...
			/* printk("fts_report_value tpd D x[%d] =%d,y[%d]= %d",i,data->au16_x[i],i,data->au16_y[i]);*/
		} else {
			up_point++;
			input_mt_report_slot_state(tpd->dev, MT_TOOL_FINGER,
						   false);
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			if (err) {
				...
				TPD_PROXIMITY_DEBUG
				    ("huang sensor_data->values[0] 1082 = %d\n",
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (tpd_proximity_flag == 1) {
			...
			TPD_PROXIMITY_DEBUG
			    ("proxi_fts 0xB0 state value is 1131 0x%02X\n",
			     state);
			if (!(state & 0x01)) {
				tpd_enable_ps(1);
			}
			...
			ret =
			    fts_read_reg(fts_i2c_client, 0x01,
					 &proximity_status);
			if (ret < 0) {
				printk("[Focal][Touch] read value fail");
				/*return ret;*/
			...
			TPD_PROXIMITY_DEBUG
			    ("proxi_fts 0x01 value is 1139 0x%02X\n",
			     proximity_status);
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	if (i >= 3) {
		force_reset_guitar();
		printk
		    ("focal--tpd reset. i >= 3  ret = %d	A3_Reg_Value = 0x%02x\n ",
		     ret, data);
		reset_flag = 1;
	...
	if (1 == flag_error) {
		printk
		    ("focal--tpd reset.1 == flag_error...data=%d	count_irq\n ",
		     data, count_irq);
		force_reset_guitar();
		reset_flag = 1;
	...
	printk
	    ("focal---------91 register value = 0x%02x	old value = 0x%02x\n",
	...
	if (0x01 == g_first_read_91) {
		...
		if (g_old_91_Reg_Value == data) {
			...
			printk
			    ("focal 91 value ==============, g_91value_same_count=%d\n",
			     g_91value_same_count);
			if (RESET_91_REGVALUE_SAMECOUNT == g_91value_same_count) {
				force_reset_guitar();
				printk
				    ("focal--tpd reset. g_91value_same_count = 5\n");
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	for (i = 0; i < 2; i++) {
		ret =
		    fts_read_reg(fts_i2c_client, buf_addr[i], (buf_value + i));
		if (ret < 0) {
			printk("[Focal][Touch] read value fail");
			/*return ret;*/
		}
...
static int __init tpd_driver_init(void)
{
	...
	i2c_register_board_info(2, &fts_i2c_tpd, 1);
