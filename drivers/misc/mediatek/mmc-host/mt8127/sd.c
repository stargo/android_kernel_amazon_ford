...
{
	static	MT65XX_POWER sd_vdd_power;
	static	MT65XX_POWER sd_io_power;
#ifdef CONFIG_MTK_EMMC_SUPPORT
#ifdef CONFIG_MTK_PMIC_MT6397
	sd_vdd_power = MT65XX_POWER_LDO_VMCH;
	sd_io_power = MT65XX_POWER_LDO_VMC;
#else
	/* MT6323 */
	sd_vdd_power = MT6323_POWER_LDO_VMCH;
	sd_io_power = MT6323_POWER_LDO_VMC;
#endif
#else
/* For Nand project, VEMC3V3 is the power of SD, VMCH is the power of MLC Nand */
#ifdef CONFIG_MTK_PMIC_MT6397
	sd_vdd_power = MT65XX_POWER_LDO_VEMC_3V3;
	sd_io_power = MT65XX_POWER_LDO_VMC;
#else
	/* MT6323 */
	sd_vdd_power = MT6323_POWER_LDO_VEMC_3V3;
	sd_io_power = MT6323_POWER_LDO_VMC;
#endif
#endif
	switch(host->id){
		case 1:
			...
			msdc_ldo_power(on, sd_io_power, VOL_3300, &g_msdc1_io);
			if (host->hw->flags & MSDC_SD_NEED_POWER) { //some board, like Yecon, need VMCH always power on, or cannot detect sd card
				msdc_ldo_power(1, sd_vdd_power, VOL_3300, &g_msdc1_flash);
			} else
			{
				msdc_ldo_power(on, sd_vdd_power, VOL_3300, &g_msdc1_flash);
			}
			break;
#if (0 == REMOVEED_FOR_MT6582)
		case 2:
...
static void msdc_dma_start(struct msdc_host *host)
{
    ...
    if(host->autocmd == MSDC_AUTOCMD12 || host->autocmd == MSDC_AUTOCMD23)
