	...
	case PROC_UPGRADE:
		{
			...
			ret =
			    fts_ctpm_fw_upgrade_with_app_file(fts_i2c_client,
							      upgrade_file_path);
#if GTP_ESD_PROTECT
			apk_debug_flag = 0;
#endif
				 ...
				 size_t count)
{
	...
	if (2 == num_read_chars) {
		...
		if (fts_read_reg(client, regaddr, &regvalue) < 0)
			printk
			    ("[Focal] %s : Could not read the register(0x%02x)\n",
			     __func__, regaddr);
		else
				  ...
				  const char *buf, size_t count)
{
	...
	/*fts_EnterUpgradeMode(client, 0 ,false); *//* : mark by focal Elmer */
...
static DEVICE_ATTR(ftsgetprojectcode, S_IRUGO | S_IWUSR,
